...
if __name__ == '__main__':
    ...
    with open('objects_last_hour.txt', 'r') as f:
        ...
        NOMBRE_COLA = 'ses-event-manager-EmailNotificationQueue-PzbmPHPzbGTO'
        bucket_name = 'febos-io'
        ...
        for object_key in single_items:
            ...
            file_bytes = s3_response[0]
            em = email.message_from_bytes(file_bytes)
            headers = get_headers(em, lower=False, camel_case=False)
            commonHeaders = get_headers(em, lower=False, camel_case=True)
            to_email, tos = get_destinations(headers, em, headers)
            attachments = process_attachments(bucket_name, em, object_key)

            messageId = get_message_id({}, headers, em)
            source = headers.get('from', '')
            recipients = to_email
            # build message eaders
            message_headers = []
            for k, v in headers.items():
                message_headers.append({"name": k, "value": v})

            mensaje = {
                "notificationType": "Received",
                "mail": {
                    "timestamp": "2024-10-07T20:41:25.754Z",
                    "source": source,
                    "messageId": messageId,
                ...
                "receipt": {
                    ...
                    "action": {
                        "type": "S3",
                        "topicArn": "arn:aws:sns:us-east-1:830321976775:ses-event-manager-EmailNotificationTopic-13Gug6rA6bdq",
                        "bucketName": "febos-io",
                        "objectKeyPrefix": "email/produccion/inbox",
                        "objectKey": object_key
                    }
                }
            }
            sqs.enviar_mensaje(NOMBRE_COLA, mensaje)
